...
static const int kResolutions[][2] = {
    {640, 480}, {800, 600}, {1024, 768}, {1280, 720}, {1600, 900}, {1920, 1080}
};

static HWND hWidthCombo, hHeightCombo, hMouseCheck, hPerfCheck, hPlayBtn, hEditBtn, hMapFileBtn, hMapCombo;
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    switch (msg) {
        case WM_CREATE: {
            ...
            CreateWindowA("STATIC", "Map:", WS_VISIBLE|WS_CHILD, 20, 110, 60, 20, hwnd, NULL, NULL, NULL);
            hMapCombo = CreateWindowA("COMBOBOX", "", WS_VISIBLE|WS_CHILD|CBS_DROPDOWNLIST, 80, 110, 120, 200, hwnd, (HMENU)IDC_MAPCOMBO, NULL, NULL);
            hMouseCheck = CreateWindowA("BUTTON", "Enable Mouse Look", WS_VISIBLE|WS_CHILD|BS_AUTOCHECKBOX, 20, 145, 180, 22, hwnd, (HMENU)IDC_MOUSE, NULL, NULL);
            hPerfCheck  = CreateWindowA("BUTTON", "Performance Mode", WS_VISIBLE|WS_CHILD|BS_AUTOCHECKBOX, 20, 170, 180, 22, hwnd, (HMENU)IDC_PERF, NULL, NULL);
            hPlayBtn = CreateWindowA("BUTTON", "Play", WS_VISIBLE|WS_CHILD|BS_DEFPUSHBUTTON, 20, 205, 180, 28, hwnd, (HMENU)IDC_PLAY, NULL, NULL);
            hEditBtn = CreateWindowA("BUTTON", "Map Editor", WS_VISIBLE|WS_CHILD, 20, 240, 180, 26, hwnd, (HMENU)IDC_EDITMAP, NULL, NULL);
            hMapFileBtn = CreateWindowA("BUTTON", "Refresh Maps", WS_VISIBLE|WS_CHILD, 20, 275, 180, 26, hwnd, (HMENU)IDC_MAPFILE, NULL, NULL);
        ...
        case WM_COMMAND: {
            if (LOWORD(wParam) == IDC_PLAY) {
                ...
                if (strlen(selectedMapFile) > 0) {
                    wsprintfA(cmd, ".\\dist\\raywin.exe -map maps\\%s -w %d -h %d %s %s", selectedMapFile, w, h, m ? "-mouselook" : "", p ? "-perf" : "--no-perf");
                } else {
                    wsprintfA(cmd, ".\\dist\\raywin.exe -w %d -h %d %s %s", w, h, m ? "-mouselook" : "", p ? "-perf" : "--no-perf");
                }
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrev, LPSTR lpCmdLine, int nCmdShow) {
    ...
    HWND hwnd = CreateWindowA(wc.lpszClassName, "RayWhen Launcher", WS_OVERLAPPED|WS_CAPTION|WS_SYSMENU|WS_MINIMIZEBOX, CW_USEDEFAULT, CW_USEDEFAULT, 240, 350, NULL, NULL, hInstance, NULL);
    if (!hwnd) return 0;
